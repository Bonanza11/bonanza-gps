...
export default async function handler(req, res) {
  ...
  */
  ...
  try {
    ...
    // 2) No permitir reprogramar si falta menos de 24h para el pickup actual
    const currentISO  = rows[0].date_iso;   // DATE
    const currentHHMM = rows[0].time_hhmm;  // 'HH:MM'
    ...
    // 3) Actualizar a la nueva fecha/hora
    const upd = await sql`
      update bookings
